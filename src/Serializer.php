...
use Zus1\Serializer\Trait\Eventable;

class Serializer
{
    use Eventable;

    public function serialize(Collection|Model|LengthAwarePaginator $subject, array|string $groups): string
    {
        $normalized = $this->normalize($subject, $groups);

        if($subject instanceof Collection || $subject instanceof LengthAwarePaginator) {
            return $normalized->toJson(JSON_PRETTY_PRINT|JSON_THROW_ON_ERROR);
        }

        return json_encode($normalized, JSON_PRETTY_PRINT|JSON_THROW_ON_ERROR);
    }
    ...
    ): array|Collection|LengthAwarePaginator
    {
        if($subject instanceof Collection || $subject instanceof LengthAwarePaginator) {
            ...
            return $nested === true ? $subject->all() : $this->withEventDispatch($subject, $className);
        }
